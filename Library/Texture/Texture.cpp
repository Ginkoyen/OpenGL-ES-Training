...
bool Texture::charger()
{
    ...
    SDL_Surface *imageInveree = inverserPixels(imageSDL);
    ...
    glBindTexture(GL_TEXTURE_2D, m_id);
        ...
        if(imageInveree->format->BytesPerPixel == 3)
        {
            // Format interne
            formatInterne = GL_RGB;

            // Format
            if(imageInveree->format->Rmask == 0xff)
                format = GL_RGB;
        ...
        else if(imageInveree->format->BytesPerPixel == 4)
        {
            // Format interne
        ...
        else
        {
            ...
            SDL_FreeSurface(imageInveree);
            return false;
        }

        // Copie des pixels
        glTexImage2D(GL_TEXTURE_2D, 0, formatInterne, imageInveree->w, imageInveree->h, 0, format, GL_UNSIGNED_BYTE, imageInveree->pixels);
